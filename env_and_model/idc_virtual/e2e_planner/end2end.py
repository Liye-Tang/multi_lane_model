...
class E2eEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward, self.reward_dict = self._compute_reward(self.obs, self.action, done, self.done_type)
        self.curr_step += 1
        all_info.update(
    ...
    def _compute_reward(self, obs, action, done, done_type):
        obses_ego, obses_track, obses_light, obses_task, obses_ref, obses_his_ac, obses_other = split_all(obs)
        reward_v = -obses_track[2] if obses_track[2] > 0 else obs[0]
        reward_cmd = -0.5 * action[0] ** 2 - 0.5 * action[1] ** 2
        reward_o = -1 if abs(obses_track[0]) > 2 else 0
        reward_c = -10 if done and done_type != 'good_done' else 0
        rewards = reward_v + reward_cmd + reward_o + reward_c
    ...
    def render(self, mode='human'):
        render(light_phase=self.light_phase, all_other=self.all_other, detected_other=self.all_detected_other,
               interested_other=self.interested_other, attn_weights=None, obs=self.obs, ref_path=self.ref_path,
