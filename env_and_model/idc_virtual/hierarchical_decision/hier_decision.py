...
from env_and_model.idc_virtual.utils.load_policy import LoadPolicy, get_args
from env_and_model.idc_virtual.utils.misc import TimerStat
from env_and_model.idc_virtual.utils.recorder import Recorder
from utils import *
...
class HierarchicalDecision(object):
    def __init__(self, exp_dir, ite, args, logdir=None):
        ...
        self.args = args
        self.env = IdcVirtualEnv(mode='testing')
    ...
    def step(self, is_render=True):
        ...
        self.hist_posi.append((self.obs[3], self.obs[4]))
    ...
    def render(self, ref_path, attn_weights, path_values):
        render(light_phase=self.env.light_phase, all_other=self.env.all_other, detected_other=self.env.all_detected_other,
...
def main():
    ...
    os.makedirs(logdir)
    exp_dir = RESULTS_DIR + '/ampc/ampc-experiment-2022-08-30-10-51-58'
    ite = 240000
...
if __name__ == '__main__':
    # main()
    logdir = os.path.dirname(CURRENT_DIR) + '/data_and_plot/single_demo/good/final'
    # plot_and_save_ith_episode_data('./results/good/2021-03-15-23-56-21', 0)
    select_and_rename_snapshots_of_an_episode(logdir, 5, 8)
