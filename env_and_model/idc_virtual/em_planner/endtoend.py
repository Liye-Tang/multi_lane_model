...
class IdcVirtualEnv(gym.Env):
    ...
    def reset(self, **kwargs):  # kwargs include three keys
        ...
        self.obs, other_mask_vector, self.future_n_point, interested_others_for_em_planner = \
            self._get_obs()
        self.action = None
        self.reward_dict = None
        self.done_type = 'not_done_yet'
        all_info = dict(future_n_point=self.future_n_point, mask=other_mask_vector,
                        ...
                        interested_others_for_em_planner=interested_others_for_em_planner)
        self.curr_step = 0
    ...
    def step(self, action):
        ...
        self.curr_step += 1
        all_info.update(
            {'reward_dict': self.reward_dict, 'future_n_point': self.future_n_point,
             'mask': other_mask_vector, 'interested_others_for_em_planner': interested_others_for_em_planner})
    ...
    def _is_achieve_goal(self):
        ...
        if self.training_task == 'left':
            return True if x < -Para.CROSSROAD_SIZE / 2 - 30 and 0 < y < Para.LANE_NUMBER * Para.LANE_WIDTH else False
        elif self.training_task == 'right':
            return True if x > Para.CROSSROAD_SIZE / 2 + 30 and -Para.LANE_NUMBER * Para.LANE_WIDTH < y < 0 else False
        else:
            assert self.training_task == 'straight'
            return True if y > Para.CROSSROAD_SIZE / 2 + 30 and 0 < x < Para.LANE_NUMBER * Para.LANE_WIDTH else False
    ...
    def _get_obs(self, exit_='D'):
        other_vector, other_mask_vector, interested_others_for_em_planner = \
            self._construct_other_vector_short(exit_)
        ...
        return vector, other_mask_vector, future_n_point, interested_others_for_em_planner
    ...
    def _construct_other_vector_short(self, exit_='D'):
        ...
        def filter_interested_other(vs, task):
            ...
            c0, c1, c2, c3 = [], [], [], []
            interested_others_for_em_planner = []
            ...
            for v in vs:
                if v['type'] in ['bicycle_1', 'bicycle_2', 'bicycle_3']:
                    interested_others_for_em_planner.append(
                        [v['x'], v['y'], v['v'], v['phi'], v['l'], v['w'], 0, 0.0])
                    ...
                    interested_others_for_em_planner.append(
                        [v['x'], v['y'], v['v'], v['phi'], v['l'], v['w'], 0, cal_turn_rad(v)])
            ...
            return tmp_bike + tmp_ped + tmp_veh, interested_others_for_em_planner

        self.interested_other, interested_others_for_em_planner = \
            filter_interested_other(self.all_detected_other, self.training_task)
    ...
    def render(self, traj=None, ref_path=None, referline_cost=None, all_obst=None):
        ax = render(light_phase=self.light_phase, all_other=self.all_other, detected_other=self.all_detected_other,
                    interested_other=None, attn_weights=None, obs=self.obs, ref_path=None,
                    ...
                    sensor_config=None)
        if traj is not None:
            s = sum([1 for x, y in zip(traj['x'], traj['y']) if -65 < x < 65 and -65 < y < 65])
            ax.plot(traj['x'][:s], traj['y'][:s], color='r')
        if ref_path is not None:
            s = sum([1 for x, y in zip(ref_path['x'], ref_path['y']) if -65 < x < 65 and -65 < y < 65])
            ax.plot(ref_path['x'][:s], ref_path['y'][:s], color='g')
        if referline_cost is not None:
            text_x, text_y = -110, 30
            ax.text(text_x, text_y,
                    'refline_cost: {:.2f}, {:.2f}, {:.2f}'.format(referline_cost[0], referline_cost[1], referline_cost[2]))
        if all_obst is not None:
            obst_x = np.clip(all_obst['x'], -65., 65.)
            obst_y = np.clip(all_obst['y'], -65., 65.)
            plt.plot(obst_x, obst_y, 'r.')
        plt.show()
        plt.pause(0.001)
