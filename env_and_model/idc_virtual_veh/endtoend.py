...
from env_and_model.idc_virtual_veh.dynamics_and_models import VehicleDynamics, ReferencePath, IdcVirtualVehModel
from env_and_model.idc_virtual_veh.endtoend_env_utils import *
from env_and_model.idc_virtual_veh.traffic import Traffic
from env_and_model.idc_virtual_veh.utils.sensor import Perception
...
class IdcVirtualVehEnv(gym.Env):
    def __init__(self,
                 mode='training',
                 multi_display=False,
                 **kwargs):
        self.mode = mode
        self.dynamics = VehicleDynamics(if_model=True)
    ...
    def reset(self, **kwargs):  # kwargs include three keys
        green_prob = kwargs['green_prob'] if 'green_prob' in kwargs else 0.9
        ...
        self.env_model = IdcVirtualVehModel()
...
def test_end2end():
    env = IdcVirtualVehEnv()
    env_model = IdcVirtualVehModel()
    obs, _ = env.reset()
    plt.ion()
    i = 0
    while i < 100000:
